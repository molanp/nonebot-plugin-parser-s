...
T2 = str.maketrans("".join(chr(i) for i in range(256)), "".join(chr((i - 2) % 256) for i in range(256)))
...
def get_final_stable_path_ultimate(text):
    ...
    if decoded_path.endswith('profile'):
        # 3. 优化 ID 查找：利用 pos 锚点跳过已处理的字符串部分
        if RE_ID.search(text, pos=match_path.end()):
